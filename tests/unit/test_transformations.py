...
class TestValidateTrackSchema:
    ...
    def test_too_long_duration_fails(self, valid_track):
        valid_track["duration_ms"] = 36_000_001  # 10 heures
        errors = validate_track_schema(valid_track)
        assert len(errors) > 0
...
class TestDeduplication:
    ...
    def test_removes_duplicate_artists_same_label(self, catalog_with_duplicates):
        artists = catalog_with_duplicates["artists"]
    ...
    def test_keeps_unique_artists(self, catalog_with_duplicates):
        artists = catalog_with_duplicates["artists"]
        result = deduplicate_artists(artists)
        # Beatles (EMI doublon supprimé) + Led Zeppelin = 2
        assert len(result) == 2


# ─────────────────────────────────────────────────────────────
# TESTS — Enrichissement des events
# ─────────────────────────────────────────────────────────────

class TestEnrichListeningEvent:

    def test_enrichment_adds_catalog_fields(self, valid_listening_event):
        catalog = {
            valid_listening_event["track_id"]: {
                "title": "Ma Chanson", "artist_id": str(uuid.uuid4()), "genre": "Pop"
            }
        }
        enriched = enrich_listening_event(valid_listening_event, catalog)
        assert enriched is not None
        assert enriched["track_title"] == "Ma Chanson"
        assert enriched["genre"] == "Pop"
        assert "artist_id" in enriched

    def test_unknown_track_returns_none(self, valid_listening_event):
        result = enrich_listening_event(valid_listening_event, {})
        assert result is None


# ─────────────────────────────────────────────────────────────
# TESTS READY-TO-RUN — Data Generator
# ─────────────────────────────────────────────────────────────
