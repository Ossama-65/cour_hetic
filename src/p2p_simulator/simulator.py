...
de streaming musical. Il publie dans Redis (Phase 1) et dans
Kafka (Phase 2).
...
# Note : On utilise DB 0 pour correspondre aux standards Docker habituels
...
class P2PSimulator:
    ...
    def __init__(
        ...
        try:
            ...
            logger.error(f"Impossible de se connecter à Redis sur {REDIS_URL} : {e}")
            exit(1)
    ...
    def _generate_listening_event(self) -> dict:
        """Génère un événement d'écoute complet et valide pour Airflow."""
        ...
        if self.mode == "late_events" and random.random() < 0.4:
            ts = datetime.utcnow() - timedelta(minutes=random.randint(5, 30))
            event["timestamp"] = ts.isoformat() + "Z"

        return event

    def _generate_p2p_network_event(self) -> dict:
        """Génère un événement technique réseau."""
        event_types = ["peer_connect", "peer_disconnect", "chunk_transfer", "cache_hit", "cache_miss"]
        ...
        event = {
            ...
            "timestamp":  datetime.utcnow().isoformat() + "Z",
            "properties": {
                "bitrate_kbps": random.choice([128, 160, 320]),
                "buffer_ms": random.randint(100, 2000)
            }
        }
        return event
    ...
    def _publish_to_redis(self, channel: str, payload: str):
        """Publie via LPUSH dans une liste Redis pour permettre le micro-batching."""
        try:
            # On utilise LPUSH pour créer une queue persistante
